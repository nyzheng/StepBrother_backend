...
// 取得所有業主資料(retuen 陣列)
async function get_owner_data() {
    try {
        const [results] = await connection.query('SELECT * FROM `owner_data`')

        const formatted_results = results.map((owner_data) => {
            owner_data.create_time = format_date_time(owner_data.create_time)
            owner_data.update_time = format_date_time(owner_data.update_time)
            return owner_data
        })
        console.log('業主資料', formatted_results)
        return formatted_results // 回傳格式化後的結果
        // 如果前端不需要格式化時間則 return results
    } catch (error) {
        console.error('無法獲取使用者資料:', error)
...
// 取得所有使用者資料"沒有密碼！"(retuen 陣列)
async function get_no_password_user_data() {
    try {
        const [results] = await connection.query('SELECT * FROM `user_data`')
